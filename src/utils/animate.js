...
const areSameEdges = (edgeElement, edge, isDirected) => {
    ...
    if (edgeElementFrom === edge.from && edgeElementTo === edge.to)
        return true;
    else if (!isDirected && edgeElementFrom === edge.to && edgeElementTo === edge.from)
        return true;
    return false;
}
...
export const glowNodes = (sequence, delay = 2000) => {
    ...
    animation.startAnimation(elementsSequence, delay, 'fill-node', 'fill-visited');
    return animation;
};
...
export const highlightPath = (sequence, delay = 2000, isDirected) => {
    ...
    const animation = new sequentialGlow();
    animation.startAnimation(elementsSequence, delay, 'stroke-edge', 'stroke-path', delay);
    return animation;
}
