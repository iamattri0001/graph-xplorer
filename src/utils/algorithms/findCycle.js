    ...
    for (let i = 0; i < adjList[node].length; i++)
        if (adjList[node][i] !== par) {
            if (vis.has(adjList[node][i])) {
                ...
                const hasCyle = checkCycleUndirected(adjList, adjList[node][i], vis, node, sequence);
                if (hasCyle) {
                    return 1;
                }
            }
...
const checkCycleDirected = (adjList, node, vis, finished, sequence) => {
    ...
    for (let i = 0; i < adjList[node].length; i++) {
        if (!vis.has(adjList[node][i])) {
            let hasCyle = checkCycleDirected(adjList, adjList[node][i], vis, finished, sequence);
            if (hasCyle) {
                return true;
            }
        } else {
